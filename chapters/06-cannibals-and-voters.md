...
We tend to think of bacteria as mindless, solitary creatures. Single cells drifting through liquid, bumping into food, dividing when conditions allow. Each one a loner, a speck of chemistry wrapped in a membrane, acting out a script written entirely by diffusion and accident.

This picture is spectacularly wrong.

Bacteria vote. They hunt in packs. They commit suicide for the good of their community. And when times get hard enough, they eat their siblings. Not as metaphor, not as a pop-science flourish---literally. The molecular machinery for each of these behaviors has been identified, cloned, knocked out, and put back in. The social life of microbes is not speculation. It is one of the best-documented stories in modern biology.
...
And from these signals, something remarkable emerged: altruism---the ability to sacrifice one's own interests for the good of the community.[^markov_altruism]
...
Here is how it works.[^ellermeier_cannibalism] [@Markov2010]
...
::: {.callout-note}
## Sidebar --- The SdpC/SdpI/SdpR immunity circuit

The cannibalism system requires a precise answer to a lethal question: how does a toxin-producing cell avoid killing itself?

The answer is a three-protein signaling pathway that couples toxin production to immunity:

- **SdpC**: the toxic protein. It is exported by cells with high Spo0A~P and kills neighboring cells that lack immunity.
- **SdpI**: the immunity protein. It is a polytopic membrane protein (spanning the membrane multiple times) that protects the cell from SdpC.
- **SdpR**: an autorepressor. In the absence of SdpC, SdpR binds the promoter of the *sdpRI* operon and blocks transcription. This keeps immunity protein levels low when the toxin is absent---a sensible economy.

The elegant twist: SdpI is not *only* an immunity protein. It is also a signal-transduction protein. When SdpC (the toxin) contacts SdpI in the membrane, SdpI responds by sequestering SdpR---physically pulling the repressor away from the DNA and trapping it at the membrane. With SdpR removed from the promoter, the *sdpRI* operon is derepressed, and the cell ramps up production of more SdpI (immunity protein).[^ellermeier_cannibalism]

The result is a self-reinforcing circuit: the presence of toxin in the environment stimulates production of immunity. Cells that are making the toxin (because they have high Spo0A~P, which drives the *sdpABC* operon) automatically encounter it and automatically induce their own immunity. Cells that are *not* making the toxin do not induce immunity fast enough and are killed.

This is a molecular implementation of a rule as old as warfare: arm yourself before you start shooting.
:::
...
There is a dark economy at work here. The population invests in a toxic public good (SdpC), splits itself into killers and victims through a stochastic switch, and recycles roughly half its members. The survivors gain time. If new nutrients arrive during that borrowed time, the entire surviving population benefits. If they do not, the survivors proceed to sporulation---but now with a head start, having fed on the remains of their kin.
...
::: {.callout-note}
## Sidebar --- Why AMP, not ATP?

It might seem more natural for the cell to measure ATP directly. In fact, AMP is a more sensitive indicator of energy stress. Here is why.

Consider a cell with a total adenine nucleotide pool (ATP + ADP + AMP) that is roughly constant. Under normal conditions, most of the pool is ATP. A small decrease in ATP causes a proportionally larger increase in AMP, because the adenylate kinase reaction ($2\,\text{ADP} \rightleftharpoons \text{ATP} + \text{AMP}$) amplifies the signal. A 10% drop in ATP can produce a several-fold increase in AMP.

AMP is therefore an amplified starvation signal. Enzymes that respond to AMP are, in effect, responding to a magnified version of the cell's energy deficit.
:::
...
::: {.callout-note}
## Sidebar --- Circuits for bistability

Two basic circuit architectures can produce bistable switching:

1. **Positive feedback**: a molecule promotes its own production. Once production exceeds a threshold, it accelerates, driving the system to a high-expression state. Below the threshold, production is too low to sustain itself, and the system falls to the low-expression state. A single positive feedback loop can generate bistability if the feedback is sufficiently nonlinear (e.g., cooperative binding).[^becskei_bistable]

2. **Double-negative feedback** (mutual inhibition): two regulators each repress the other. If A is dominant, it suppresses B, which reinforces A's dominance. If B is dominant, it suppresses A. The system has two stable states (A-on/B-off and A-off/B-on) and resists intermediate states.[^gardner_toggle]

Both architectures share two properties:

- **Persistence**: once the switch has flipped, it stays flipped. The state is self-maintaining, even after the triggering signal is removed. This is how a transient signal (e.g., a brief pulse of starvation) can produce a permanent change in cell fate.
- **All-or-none response**: the transition between states is sharp. Cells do not end up "half-sporulated" or "partially committed." They are in one state or the other.
...
Step back now and consider what we have assembled.
...
This is, functionally, a democracy. Not the kind with ballot boxes and campaign speeches, but the kind that matters in physics: a system where local interactions produce global order, where individual "votes" (chemical signals) are integrated into collective "decisions" (population-level phenotypic switches), and where the outcome is not dictated by any central authority but emerges from the distributed computation of thousands of cells, each following the same molecular rules.

## The ancient social contract

Here, then, is the philosophical bombshell.
...
[^markov_altruism]: Alexandr Markov, *Birth of Complexity. Evolutionary Biology Today: Unexpected Discoveries and New Questions* (2010). [@Markov2010]

[^markov_quorum]: Markov describes quorum sensing as "a kind of chemical voting, when a certain critical number of votes changes the behavior of bacteria." [@Markov2010]
